...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen w-full bg-background overflow-hidden">
      ...
      <aside 
        className={cn(
          "flex flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground transition-all duration-300 z-20",
          isSidebarOpen ? "w-64" : "w-[72px]"
        )}
      >
        <div className="flex h-16 items-center px-4 border-b border-sidebar-border/50 gap-3">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-background relative">
        {/* Header */}
        <header className="h-16 flex items-center justify-between px-6 border-b border-border/50 bg-card/50 backdrop-blur-md sticky top-0 z-10">
